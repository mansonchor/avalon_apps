define('wo/module/common_function',['wo_config','base_package','ua',"new_alert_v2","app_function"],function(require, exports)
{
    ...
    */
    ...
    // 截取图片居中部分
    exports.img_center = function(img_obj,target_height,target_width,after_onload_change)
    {   
        var that = this;
        
        var after_onload_change = (after_onload_change == null)?true:false;
        
        if(img_obj)
        {
            target_height = parseInt(target_height);
            target_width  = parseInt(target_width);
            ...
            img_obj.onload = function()
            {
                ...
                if( (imgWidth/target_width) > (imgHeight/target_height) )
                {
                    var retWidth = target_height*imgWidth / imgHeight;
                    img_obj.style.width = retWidth;
                    img_obj.style.height = target_height + 'px';
                    img_obj.style.marginLeft = -(retWidth-target_width)/2 + "px";
                }
                else
                {
                    var retHeight = target_width*imgHeight / imgWidth;
                    img_obj.style.height = retHeight;
                    img_obj.style.width  = target_width + 'px';
                    img_obj.style.marginTop = -(retHeight-target_height)/2 + "px";
                }	
            ...
            img_obj.onerror = function()
            {
                ...
                that.img_center(img_obj,target_height,target_width) 
            }
    ...
    exports.oau_login = function(login_tag,after_login_go_to,logining,callback,error)
    {
        ...
        function is_not_sso_login(req_url)
        {
            if(after_login_go_to)//_state && _state.url_form_font_page
            {
                ...
                console.log(default_index_route)

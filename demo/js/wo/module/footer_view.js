define("wo/module/footer_view",["base_package",'frame_package',"wo_config","commom_function","get_template","notice"],function(require, exports)
{
	...
	({
		...
		events : {
			...
			{	
                var category_nav_str = window.localStorage.getItem('category_nav')
                
                if(!common_function.is_empty(category_nav_str))
                {
                    page_control.navigate_to_page(category_nav_str)     
                }
                else
                {
